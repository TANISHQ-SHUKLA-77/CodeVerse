...
export async function createQuestion(
  ...
  if (!userId) {
    return handleError(new Error("User ID is required")) as ErrorResponse;
  }

  // Ensure userId is a valid ObjectId
  let authorId: Types.ObjectId;
  
  // Check if userId is already a valid ObjectId format
  if (Types.ObjectId.isValid(userId)) {
    authorId = new Types.ObjectId(userId);
  } else {
    // If not valid ObjectId, try to find user by email
    // This handles cases where session might have invalid format
    const User = (await import("@/database/user.model")).default;
    const userEmail = validationResult.session?.user?.email;
    
    if (!userEmail) {
      return handleError(new Error("User email not found. Please sign in again.")) as ErrorResponse;
    }
    
    const user = await User.findOne({ email: userEmail }).select("_id");
    
    if (!user) {
      return handleError(new Error("User not found. Please sign in again.")) as ErrorResponse;
    }
